...
class ConvNetDrop(nn.Module):
    def __init__(self, depth, flatten = True):
        ...
        for i in range(depth):
            ...
            trunk.append(B)
            # if i > 1: # Last 2 layers Drop 50% neurons
...
class ConvNetManifold(nn.Module):
    def __init__(self, depth = 4, flatten = True, num_classes = 200):
        super(ConvNetManifold, self).__init__()
...
def ResNet10Drop(flatten = True):
    return ResNetDrop(SimpleBlock, [1,1,1,1],[64,128,256,512], flatten)
...
def Convnetmanifoldmixup():
    return ConvNetManifold(4)
