...
class BaselineTrainMixUp(nn.Module):
    def __init__(self, model_func, num_class, alpha = 1.0, loss_type = 'softmax'):
        super(BaselineTrainMixUp, self).__init__()
    ...
    def forward_loss(self, x, y_a, y_b, lam):
        scores = self.forward(x)
        y_a = Variable(y_a.cuda())
        y_b = Variable(y_b.cuda())
        return self.loss_fn(self.criterion, scores, y_a, y_b, lam)
    ...
    def train_loop(self, epoch, train_loader, optimizer):
        ...
        for i, (x,y) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            loss = self.forward_loss(x, y_a, y_b, lam)
            loss.backward()
            optimizer.step()
