...
class BaselineTrainManifoldMixup(nn.Module):
    def __init__(self, model_func, num_class, mixup=False, mixup_hidden=True, mixup_alpha=2.0, loss_type = 'softmax'):
        super(BaselineTrainManifoldMixup, self).__init__()
    ...
    def forward(self,x, y):
        y    = Variable(y.cuda())
        x    = Variable(x.cuda())
        out, reweighted_y  = self.feature.forward(x = x, target = y, mixup = self.mixup, mixup_hidden = self.mixup_hidden, mixup_alpha = self.mixup_alpha)
        scores  = self.softmax(self.classifier.forward(out))
        return scores, reweighted_y

    def forward_loss(self, x, y):
        scores, reweighted_y = self.forward(x, y)
        reweighted_y = Variable(reweighted_y.cuda())
        return self.loss_fn(scores, reweighted_y)
